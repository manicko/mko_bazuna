...
def _load_bindings(
    ...
    if apps is not None:
        ...
        CategoryListingCondition = apps.get_model("categories", "CategoryListingCondition")  # noqa: N806
        LookupItem = apps.get_model("lookups", "LookupItem")  # noqa: N806
    else:
        from apps.categories.models import CategoryListingCondition, CategoryListingFeature, CategoryListingPurpose  # type: ignore[no-redef]
    ...
    def _process_bindings(items: list[dict[str, Any]]) -> None:
        nonlocal count_purposes, count_features, count_conditions
        ...
        for item in items:
            ...
            # Bind listing conditions
            condition_overrides = item.get("listing_condition_override", [])
            if condition_overrides:
                for c_slug in condition_overrides:
                    try:
                        condition_item = LookupItem.objects.get(
                            slug=c_slug,
                            group__code="listing_condition",
                        )
                        CategoryListingCondition.objects.update_or_create(
                            category=category,
                            listing_condition=condition_item,
                        )
                        count_conditions += 1
                    except LookupItem.DoesNotExist:
                        logger.warning(
                            "Listing condition slug not found: %s", c_slug
                        )
    ...
    logger.info("Loaded %d purpose bindings, %d feature bindings, %d condition bindings", count_purposes, count_features, count_conditions)
