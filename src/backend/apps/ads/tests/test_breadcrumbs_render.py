...
class TestBreadcrumbsRender(TestCase):
    ...
    def test_breadcrumb_shows_root_category(self) -> None:
        ...
        self.assertIn("Главная", nav)
        self.assertIn("Бизнес", nav)

    def test_breadcrumb_shows_ancestor_chain(self) -> None:
        """A child category renders its ancestor chain (RC-C: no crash on
        ancestors, ellipsis branch not required for a single ancestor)."""
        response = self.client.get("/category/business-commercial-real-estate/")
        self.assertEqual(response.status_code, 200)
        nav = _breadcrumb_nav(response.content.decode("utf-8"))
        self.assertIn("Бизнес", nav)
        self.assertIn("Коммерческая недвижимость", nav)

    def test_breadcrumb_on_ad_detail(self) -> None:
        ...
        response = self.client.get(reverse("ads:detail", args=[ad.id]))
        self.assertEqual(response.status_code, 200)
        nav = _breadcrumb_nav(response.content.decode("utf-8"))
        # business -> business-commercial-real-estate -> business-offices
        self.assertIn("Бизнес", nav)
        self.assertIn("Коммерческая недвижимость", nav)
        self.assertIn("Офисы", nav)
