...
class PreferredCityMiddlewareTests(SimpleTestCase):
    """Tests for PreferredCityMiddleware resolution priority."""

    def setUp(self) -> None:
        self.middleware = PreferredCityMiddleware(get_response=lambda r: None)

    # --- Authenticated: DB wins ---

    def test_authenticated_db_wins_over_cookie(self) -> None:
    ...
    def test_authenticated_without_db_preference_falls_back_to_cookie(self) -> None:
    ...
    def test_anonymous_valid_cookie(self) -> None:
    ...
    def test_anonymous_absent_cookie_is_none(self) -> None:
    ...
    def test_anonymous_stale_cookie_is_none(self) -> None:
    ...
    def test_stale_cookie_deleted_in_response(self) -> None:
        ...
        self.assertIn(PREFERRED_CITY_COOKIE_NAME, response.cookies)
        self.assertEqual(response.cookies[PREFERRED_CITY_COOKIE_NAME].value, "")

    def test_valid_cookie_not_deleted(self) -> None:
        ...
        with patch(
            ...
            self.middleware.process_request(request)
            self.middleware.process_response(request, response)
        self.assertNotIn(PREFERRED_CITY_COOKIE_NAME, response.cookies)

    # --- Ordering guard ---

    def test_no_crash_when_user_absent(self) -> None:
    ...
    def test_middleware_never_writes_cookie(self) -> None:
