...
``migrate --run-syncdb``, ``setup_search_triggers``, and ``load_exchange_rates``
...
class Command(BaseCommand):
    ...
    ``load_exchange_rates`` in sequence inside ``AdvisoryLockId.MIGRATE`` (100)
    so that all post-migration setup is serialized by a single advisory lock,
    replacing the previous ``&&``-chained shell command that released the lock
    between steps.
    """
    ...
    def handle(self, *args, **options) -> None:
        ...
        ``migrate_locked.main()`` runs all three post-migration steps as
        subprocesses under the session-scoped ``MIGRATE`` lock and returns the
        first non-zero exit code (or ``0`` on full success). We re-raise as
        ``SystemExit`` so Docker one-shot services and CI observe the real
        exit code, preserving the fail-closed semantics of the original chain.
        """
