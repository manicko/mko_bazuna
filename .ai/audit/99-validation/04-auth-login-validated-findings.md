...
**Recommendation (Option A — accept and document, NO code change):** Treat the existing 192-bit/32-char token as the project standard. `secrets.token_urlsafe(24)` at `consent.py:298` already exceeds NIST SP 800-63B §5.1.1.2's 128-bit authenticator minimum and is protected in depth by: SHA-256 hash-only storage (`token_hash CHAR(64)`, `models.py:162-166`), 5-minute `expires_at` (5-min TTL), per-IP rate limiting (10 req/min via `login_rate_limit_check`, `consent.py:294`), and single-use atomic claim via `UPDATE…RETURNING` (`login.py:119-131`). The 32-char length is architecturally coupled: the bot regex `LOGIN_PATTERN` at `login.py:27` (`{32}`) must match the web generator at `consent.py:298` (`token_urlsafe(24)`), and the same 32-char / `token_urlsafe(24)` convention is used project-wide for unsubscribe tokens (`alerts.py:28` `UNSUB_DEEPLINK_PATTERN` `{32}` + `SavedSearch.unsubscribe_token` generation at `models.py:151`). **Option B is rejected** (upgrade to `token_urlsafe(32)` → 43-char): it would require atomically deploying the generator (`consent.py:298`) and the bot regex (`login.py:27`) across two independently-started processes (web gunicorn + bot aiogram) with no safe partial-deploy order — web-first emits 43-char tokens the `{32}` regex rejects, and bot-first won't match existing 32-char tokens — plus test churn (`test_login.py:92` length assertion `len(...) == 32`, `login_token_factory` fixture at `conftest.py:114`) — all for zero security gain over a design that already exceeds the NIST floor. Update docs to state the entropy floor explicitly: (1) `docs/01-spec/spec-index.md:75` → change "32-char" to "32-char URL-safe token (~192-bit CSPRNG, rate-limited 5-min TTL — exceeds NIST SP 800-63B 128-bit minimum)"; (2) `docs/02-database/db-schema.md:84` → append "(192-bit CSPRNG; exceeds NIST SP 800-63B §5.1.1.2 128-bit authenticator minimum)" to the `token_hash` comment; (3) `docs/01-spec/technical-specification.md:141` → append the same NIST rationale to the "32-char URL-safe token" note. **Effort: small (docs only, zero code/test changes).**
...
- **AU-002 (Option A, chosen)** — Accept 192-bit/32-char as the project standard (NO code change). Update `docs/01-spec/spec-index.md:75`, `docs/02-database/db-schema.md:84`, and `docs/01-spec/technical-specification.md:141` to state "32-char URL-safe token (~192-bit CSPRNG, rate-limited 5-min TTL — exceeds NIST SP 800-63B's 128-bit authenticator minimum)". Option B (upgrade to 256-bit via `token_urlsafe(32)` + regex `{32}`→`{43}`) is rejected: the 32-char length is architecturally coupled to the bot regex (`login.py:27`) and the project-wide unsubscribe-token convention (`alerts.py:28`, `models.py:151`); a 256-bit upgrade would require atomic cross-process deployment of `consent.py:298` + `login.py:27` across independently-deployed web/bot processes with no safe partial-deploy order, plus test churn, for zero security gain over a NIST-compliant 192-bit design.
...
- **AU-002** — `docs/01-spec/spec-index.md:75`, `docs/02-database/db-schema.md:84`, `docs/01-spec/technical-specification.md:141`: state the exact entropy (192-bit / 32-char, exceeding NIST SP 800-63B's 128-bit minimum). No upgrade to 256-bit (Option A accepted).
...
- **AU-002 (advisory, docs-only chosen — Option A):** No production code or test changes. Docs update (`spec-index.md:75`, `db-schema.md:84`, `technical-specification.md:141`) to state the 192-bit/32-char floor with NIST SP 800-63B rationale. **Zero rollout risk** — pure documentation alignment with the already-deployed, NIST-compliant token. Option B (upgrade to 256-bit via `secrets.token_urlsafe(32)` + regex `{32}`→`{43}`) was evaluated and **rejected** because it requires atomically deploying the generator (`consent.py:298`) and the bot regex (`login.py:27`) across two independently-started processes (web gunicorn + bot aiogram) with no safe partial-deploy order — neither rollout order tolerates in-flight tokens, and existing 32-char tokens in the 5-min TTL window would fail the new `{43}` regex. The 32-char length is also a project-wide convention shared with unsubscribe tokens (`alerts.py:28`, `models.py:151`), so fragmenting it for login alone would create inconsistency.
...
| AU-002 | Yes — `consent.py:298` (`secrets.token_urlsafe(24)`), `login.py:27` (`LOGIN_PATTERN` `{32}`), `docs/01-spec/spec-index.md:75`, `docs/02-database/db-schema.md:84`, `docs/01-spec/technical-specification.md:141` | Yes (grep + file reads; 24 bytes→192 bits→32 chars; SHA-256 hex=64 chars) | Yes — trivial (docs only, zero code changes) |
...
- **AU-002 (rejected Option B):** Bumping to 256-bit would require the bot regex `login.py:27` to move from `{32}` to `{43}` and the deep-link generator (`consent.py:307`) to emit the longer raw token into `?start=login_<raw_token>` — both ends must change together across independently-deployed processes with no safe rollout order, and existing 32-char tokens in the 5-min TTL window would fail the new `{43}` regex. Additionally, the 32-char / `token_urlsafe(24)` convention is shared project-wide with unsubscribe tokens (`alerts.py:28`, `models.py:151`), so fragmenting it for login alone would create inconsistency. **Mitigation (if Option B were ever adopted):** generate tokens via a shared `token_urlsafe(32)` helper used by both login and unsubscribe, run a 5-min TTL drain window, then flip both regexes together — but the security gain is nil (192 bits already exceeds NIST 128-bit floor), so Option A is preferred.
...
1. **AU-002 (Option A):** No production code or test changes. Update `docs/01-spec/spec-index.md:75`, `docs/02-database/db-schema.md:84`, and `docs/01-spec/technical-specification.md:141` to state "32-char URL-safe token (~192-bit CSPRNG, rate-limited 5-min TTL — exceeds NIST SP 800-63B's 128-bit authenticator minimum; only SHA-256 hash stored; single-use atomic claim)". Rationale: 192 bits already exceeds the NIST SP 800-63B 128-bit authenticator floor and is defended by hash-only storage, 5-min TTL, per-IP rate limiting, and atomic `UPDATE…RETURNING` claim — Option B (upgrade to `token_urlsafe(32)`→43-char) is rejected due to cross-process atomicity risk (`consent.py:298` generator + `login.py:27` regex must change together across independently-deployed web/bot processes with no safe rollout order) and inconsistency with the project-wide 32-char token convention (`alerts.py:28`, `models.py:151`).
