...
> - **Recommendation:** Fix the template to use the existing `search:search` URL name. The save-search feature is a planned Phase 2 capability (`.ai/plans/02/Saved Search Alerts/plan.md`, "Phase 2, Ready for Implementation", task T7), not yet implemented — no `save_search` view or `search:save-search` URL exists in `src/`. Immediate fix: change line 6 `{% url 'search:list' %}` to `{% url 'search:search' %}` so the partial renders without `NoReverseMatch`. The `search:save-search` POST action (line 14) corresponds to the future Phase 2 endpoint; since the modal is not currently included in any view, it causes no runtime error. Do NOT retarget the form POST to `search:search` (a GET-only view) — implement the dedicated endpoint as part of Phase 2.
...
> - **Recommendation:** Remove `invalidate_translation_cache()` as dead code — it has zero call sites (grep-confirmed) and no dynamic translation settings exist to warrant cache invalidation. The LRU caches are bounded (maxsize 128 and 256), so stale entries are evicted automatically. Concrete steps: (1) delete `invalidate_translation_cache()` (lines 146-149 of `query_translator.py`); (2) update the `translate_cached_generic` docstring (line 157) to remove the dangling reference to `invalidate_translation_cache()` — replace with a note that the LRU cache evicts by bounded capacity; (3) the existing test fixture (`test_query_translator.py` line 32) already clears `translate_cached` directly and needs no change.
...
### SRH-006 (template URL fix)
- **Risk:** LOW. Fix the broken `{% url 'search:list' %}` reference in `save_search_modal.html` (line 6) to use the existing `search:search` URL name. The `search:save-search` POST action (line 14) is a planned Phase 2 endpoint (task T7 in `.ai/plans/02/Saved Search Alerts/plan.md`); no view/URL exists yet and the modal is not currently rendered, so it causes no runtime error. Implement the dedicated save-search endpoint as part of Phase 2.
- **Rollout:** Trivial template change; safe. No migration required.

### SRH-007 (test cache isolation)
- **Risk:** NONE. Test-only change.

### SRH-008 (dead code removal)
- **Risk:** NONE. Remove the unused `invalidate_translation_cache()` function from `query_translator.py` (lines 146-149) and update the `translate_cached_generic` docstring (line 157) to remove the dangling reference. No call sites exist; LRU caches use bounded eviction (maxsize 128/256).
- **Rollout:** Trivial delete; safe. No migration required.
...
9. **SRH-008** (dead code removal � optional)
