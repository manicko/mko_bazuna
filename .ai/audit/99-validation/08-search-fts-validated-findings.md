...
**Recommendation:** Per the dead-code policy, investigate the column's intended purpose first: (1) if a documented "match-any-language" fallback is wanted, wire `search_vector` + `IX_ads_search_gin` to an explicit fallback query path and document it, or (2) if unused, remove the column, its GIN index, and the 7 `to_tsvector` assignments for `search_vector` from the trigger function (plus the backfill RunSQL). Effort: medium. Priority: recommended. **Validator addendum:** Given the column is a documented Phase-3-transitional artifact, option (2) should be gated on (and coordinated with) the Phase 3 migration plan; track the 7/16 write-amp cost as a Phase-3 milestone item rather than removing out-of-band.
