...
**Rollout Safety:** Incremental, low-risk. Replace each blanket ignore with a scoped comment + rationale, or wrap in a typed helper. Does not change runtime behavior.
...
4. **QLT-004 (MEDIUM)** — Replace blanket `# pyright: ignore[reportGeneralTypeIssues]` with scoped ignores (`reportGeneralTypeIssues` named rules + `# Django: ...` rationale) or typed wrappers across the 29 sites. Effort: small.
...
- **QLT-004 is a pure refactor** (comments/wrappers) — safe to apply anywhere in the sequence; no behavioral impact.
