...
> **Validation Correction (2026-09-05):** This document has been updated to resolve contradictions identified by the Validator and Auditor. Four surgical corrections applied: (1) **ME-002** recommendation updated to REUSE `AdImage.storage_keys()` (Phase 06 PC-004 abstraction) instead of duplicating 4-key collection across 6 sweep files — collapsing 7 maintenance points into 1 model method, with a cross-reference to PC-004; (2) **ME-001** recommended regex tightened to `^[A-Za-z0-9][A-Za-z0-9._-]*(/[A-Za-z0-9][A-Za-z0-9._-]*)*\.jpg$` (the original permits `..` via the dot in the char class) and clarified that the regex is defense-in-depth format guidance, **not** the security boundary — the `os.path.realpath` containment check in `delete_photo` is the real boundary; (3) **ME-003** cross-reference note added stating that Phase 03 DB-004's outbox recommendation (`MediaPendingDelete` model + `AdvisoryLockId.MEDIA_REAPER = 13`) is formally superseded by the sweep-fix approach (NEW-ME-007), and should NOT be implemented; (4) **NEW-ME-007** secondary recommendation to route the sweep's `os.remove` through `delete_photo` is deferred to post-ENT-001 relocation — it must use the *moved* `apps/media/services/filesystem.py` location, not the current bot-path `delete_photo`, to avoid perpetuating the backend→bot reverse dependency. All other content preserved.

> **🔄 Validation Correction (lock-ID 103 collision, 2026-09-05):**
>
> **Issue:** ENT-039 (Phase 05 catalog-seeding) and NEW-ME-007 (Phase 07 media) both claimed advisory-lock ID **103**: `CATALOG_LOAD = 103` (proposed per `ENT-031-036-research.md:293`) and `SWEEP_ORPHANED_MEDIA = 103` (documented in `sweep_orphaned_media.py:13` docstring and recommended in this file's NEW-ME-007 section).
>
...
>
> **Resolution:** ID **103** is retained for `SWEEP_ORPHANED_MEDIA` (the orphan sweep — its docstring already claims it and the research confirmed it as the correct home for the reconciliation backstop); `CATALOG_LOAD` (ENT-039, Phase 05) is reassigned to ID **104**. This file retains `SWEEP_ORPHANED_MEDIA = 103` throughout. The cross-referenced resolution is documented in `05-catalog-seeding-validated-findings.md` (ENT-039 section).
>
> **Additional recommendation:** Add a CI assertion in `test_advisory_lock_ids.py` that every `AdvisoryLockId.*` referenced at a call site is defined in the enum — this would have caught the currently-broken `sweep_orphaned_media.py:87` (`AttributeError` at runtime because `SWEEP_ORPHANED_MEDIA` was missing from the enum when the command was written).
...
| ME-003 vs. Phase 03 (DB-004) | Phase 03 DB-concurrency audit already identified the same non-atomic DB/delete pattern in `delete_sweep.py`, `sweep_drafts.py`, and `consent_hard_delete.py` (findings DB-004/DB-003). Its validated report (line 227) notes `delete_photo` already has 3-attempt retry and swallowed errors. ME-003 partially duplicates this finding. | **Resolved — superseded.** ME-003's sweep-fix approach (NEW-ME-007) is formally selected over DB-004's outbox recommendation. Phase 03 DB-004's outbox model (`MediaPendingDelete`) + `AdvisoryLockId.MEDIA_REAPER = 13` should NOT be implemented. DB-004's root-cause analysis (non-atomic DB-then-delete) remains valid as diagnostic context. |
| ENT-039 (Phase 05) vs. NEW-ME-007 (lock-ID collision) | Phase 05 ENT-039 originally proposed `CATALOG_LOAD = 103` (next free slot after `BACKFILL_THUMBNAILS=102`, per `ENT-031-036-research.md:293`). NEW-ME-007 claims `SWEEP_ORPHANED_MEDIA = 103` (its docstring at `sweep_orphaned_media.py:13` already documents "advisory lock 103"). `pg_advisory_lock` IDs are session-global — sharing 103 between the catalog cold-start one-shot and an hourly filesystem sweep means a stale lock in one domain deadlocks the other; since `load_catalog`/`load_cities` gate web/bot startup, a stale `sweep_orphaned_media` lock would block production boot. | **Resolved:** ID **103** is reserved for `SWEEP_ORPHANED_MEDIA`; `CATALOG_LOAD` is reassigned to **104**. This file retains `SWEEP_ORPHANED_MEDIA = 103`. Cross-reference: `05-catalog-seeding-validated-findings.md` (ENT-039). Also recommends a CI assertion that every `AdvisoryLockId.*` call-site reference is defined in the enum (would have caught the broken `sweep_orphaned_media.py:87`). |
...
**Recommendation:** [BEST-PRACTICE] Add defense-in-depth at the media primitive: in `delete_photo`, canonicalize with `os.path.realpath` and assert the result is within `os.path.realpath(MEDIA_ROOT)`; reject keys containing NUL, a leading `/`, or `..` components. Enforce the root cause with a `RegexValidator`/`CheckConstraint` on `AdImage.image` and `thumbnail_*` matching `^[A-Za-z0-9][A-Za-z0-9._-]*(/[A-Za-z0-9][A-Za-z0-9._-]*)*\.jpg$` (allowing the documented `seed/<name>.jpg` namespace; each path segment must start with an alphanumeric character, which inherently rejects `..` traversal segments). Never rely on the DB lookup as the sole traversal barrier.

> **Defense-in-depth clarification:** The `os.path.realpath` containment check in `delete_photo` (to be added post-ENT-001 relocation at `apps/media/services/filesystem.py`) is the **primary security boundary** that prevents CWE-22 path traversal — it canonicalizes the resolved path and verifies it stays within `os.path.realpath(MEDIA_ROOT)`, regardless of what the regex permits. The `RegexValidator`/`CheckConstraint` is a **secondary, defense-in-depth format guide** enforced at the model/ORM layer: it prevents poisoned keys from persisting in the database at rest, reducing the attack surface, but must **not** be relied upon as the sole traversal barrier. The original regex `^[A-Za-z0-9._-]+(\/[A-Za-z0-9._-]+)*\.jpg$` was rejected during validation because `.` is in the character class `[A-Za-z0-9._-]`, permitting `..` segments (e.g. `../etc/passwd.jpg` would pass). The tighter pattern above requires every path segment to start with an alphanumeric character `[A-Za-z0-9]`, which inherently rejects `..`, leading `/`, and `/etc/passwd.jpg`-style attacks — all without needing Perl-style lookahead (which PostgreSQL POSIX ERE does not support).
...
**Recommendation:** [SPEC-DEVIATION] Have each sweep collect all four physical keys per AdImage — `image`, `thumbnail_small`, `thumbnail_medium`, `thumbnail_large` (filtered to non-null) — and pass each to `delete_photo`. To avoid 7 divergent maintenance points across the 6 sweep commands + `soft_delete_user_ads`, the sweeps must **REUSE** the `AdImage.storage_keys()` model method proposed by Phase 06 (PC-004): implement `storage_keys()` on `AdImage` first (per PC-004), then have all 6 sweeps + `soft_delete_user_ads` + `consent_hard_delete` call it. This collapses 7 copy-pasted `values_list` calls into a single model method, reducing future drift. For deduplicated/shared seed thumbnails, gate deletion on a reference-count query (seed thumbnails are not in any `AdImage` row; see `sweep_orphaned_media.py`'s `seed/` directory exclusion for the existing pattern). Add a regression test asserting thumbnail files are removed after a sweep. Also fix `sweep_orphaned_media.py` (NEW-ME-007) to make the reconciliation backstop functional and schedule it.

> **Cross-reference — Phase 06 PC-004:** PC-004 (validated, `.ai/audit/99-validation/06-pii-consent-validated-findings.md:110-138`) proposed adding `AdImage.storage_keys()` returning all non-empty keys (`image` + any set `thumbnail_*`), to be used in `soft_delete_user_ads` and `consent_hard_delete`. The Validator confirmed `storage_keys()` does NOT yet exist on `AdImage` (grep `def storage_keys` → 0 matches). ME-002 must not duplicate PC-004's logic with inline 4-key collection across 6 sweep files. Instead, implement `storage_keys()` on `AdImage` first (satisfying PC-004), then have all 6 sweeps + `soft_delete_user_ads` + `consent_hard_delete` call it from a single model method. This collapses 7 maintenance points into 1 and pre-empts the duplication risk flagged by the Auditor (§3) and Validator.
...
1. **`src/backend/apps/core/enums.py`** — Add `SWEEP_ORPHANED_MEDIA = 103` to the `AdvisoryLockId` enum (the enum currently has IDs 1–12, 10, 100–111; 103 is a free slot). The command's own docstring (line 13) already documents "Uses advisory lock 103." **Cross-phase reconciliation:** Phase 05's `CATALOG_LOAD` was reassigned from 103 to **104** to avoid this collision — 103 is now exclusively `SWEEP_ORPHANED_MEDIA` (see the Cross-Phase Reconciliation note at the top of this file and the ENT-039 section in `05-catalog-seeding-validated-findings.md`).
...
**Secondary measures:**
- Once ME-001's `os.path.realpath` containment check lands in `delete_photo`, route the sweep's direct `os.remove` call (sweep_orphaned_media.py:112) through `delete_photo` (or a shared safe-deletion primitive) to inherit the containment check — the sweep currently bypasses `delete_photo` and uses raw `os.remove`, leaving it vulnerable to the same path-traversal risk. **DEFERRED to post-ENT-001 relocation:** `delete_photo` currently lives in `telegram_bot/services/media.py` (bot package); routing it from the backend sweep `sweep_orphaned_media.py` would create a backend→bot reverse dependency (the exact violation ENT-001 eliminates). After the ENT-001 move relocates `delete_photo` to `apps/media/services/filesystem.py` (same `apps/media` namespace as the sweep), the import direction is correct and the routing can land safely.
...
> **Cross-reference — Phase 03 DB-004 superseded:** Phase 03 DB-004 (validated, `99-validation/03-db-concurrency-validated-findings.md:183`) recommended **Approach A** — a new `MediaPendingDelete` outbox model + `reap_pending_deletes` command with `AdvisoryLockId.MEDIA_REAPER = 13`. This recommendation is **formally superseded** by ME-003's sweep-fix approach (NEW-ME-007): the reconciliation sweep (`sweep_orphaned_media.py`, `AdvisoryLockId.SWEEP_ORPHANED_MEDIA = 103`) provides a superset of the outbox's coverage (crash orphans, thumbnail orphans, partial-batch orphans, and arbitrary escaped files) with zero new model/migration/daemon surface area. The outbox model `MediaPendingDelete` and `AdvisoryLockId.MEDIA_REAPER = 13` **should NOT be implemented**. DB-004's root-cause analysis (non-atomic DB-then-file-delete in sweeps) remains valid as diagnostic context; the sweep is the recovery path for that risk. See ME-003 Research Findings §1–§3 (below) for the full rationale.
...
**Recommendation:** Add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` in enums.py (**ID 103 is retained for the orphan sweep; Phase 05's `CATALOG_LOAD` was reassigned to **104** to avoid this collision — see the Cross-Phase Reconciliation note at the top of this file**). Add `sweep_orphaned_media` to the hourly scheduler loop in `docker/entrypoint-scheduler.sh`. Add a test in `test_sweep_lock_structure.py`. **CI assertion:** Add a CI test in `test_advisory_lock_ids.py` that asserts every `AdvisoryLockId.*` referenced at a call site is actually defined in the enum — this would have caught the currently-broken `sweep_orphaned_media.py:87` at the time the command was authored. The orphan sweep uses `os.remove` directly (not `delete_photo`) — **DEFERRED to post-ENT-001 relocation**: routing the sweep's `os.remove` through `delete_photo` must wait until `delete_photo` is moved from `telegram_bot/services/media.py` (bot package) to `apps/media/services/filesystem.py` (backend `apps/media` namespace, per Phase 01 ENT-001). Only at that point does the import direction (backend sweep → backend `filesystem.py`) become correct; importing the bot-path `delete_photo` into the backend sweep would create a backend→bot reverse dependency — the exact violation ENT-001 eliminates. After the move, call `delete_photo(key)` (from `apps.media.services.filesystem`) to inherit the ME-001 containment check.
...
1. **ME-001 (CRITICAL) — Fix path traversal in `delete_photo`.** Add `os.path.realpath` containment check in `delete_photo` (to be applied at the ENT-001-moved location `apps/media/services/filesystem.py`, not the current bot-path `media.py`); add `RegexValidator`/`CheckConstraint` on `AdImage.image` and `thumbnail_*` fields matching `^[A-Za-z0-9][A-Za-z0-9._-]*(/[A-Za-z0-9][A-Za-z0-9._-]*)*\.jpg$` to enforce the documented UUID+seed format at rest (tighter pattern rejects `..` segments — see ME-001 defense-in-depth note above). Apply the same containment check to `AdImage.save()` (models.py:584). **Routing the sweep's `os.remove` through `delete_photo` is DEFERRED** to post-ENT-001 relocation — see NEW-ME-007. Atomic: modifies one function + model fields + import sites in the 6 sweep commands for the validator. **No runtime risk for valid keys.**

2. **NEW-ME-007 (HIGH) — Fix broken `sweep_orphaned_media` command.** Add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` in `enums.py` (ID 103 is retained for the orphan sweep; Phase 05's `CATALOG_LOAD` was reassigned to 104 to avoid this collision — see the Cross-Phase Reconciliation note at the top of this file and the ENT-039 section in `05-catalog-seeding-validated-findings.md`) and add `sweep_orphaned_media` to the hourly scheduler loop in `docker/entrypoint-scheduler.sh`. Add a test in `test_sweep_lock_structure.py`. **Critical prerequisite for ME-003's reconciliation backstop.**
...
5. **ME-003 (HIGH) — Escalate swallowed deletion failures.** After fixing NEW-ME-007, the orphan sweep provides crash-recovery reconciliation. Additionally: (a) escalate `delete_photo`'s exhausted-retry `logger.error` to a structured metric/event. **The Phase 03 DB-004 outbox approach (`MediaPendingDelete` model + `AdvisoryLockId.MEDIA_REAPER = 13`) is formally superseded** by this sweep-fix approach — see ME-003 cross-reference note above. Medium effort; should be done in the same PR as ME-001's `delete_photo` changes (both modify the same function).

6. **ME-002 (HIGH) — Collect thumbnail keys in sweeps via `storage_keys()`.** Implement `AdImage.storage_keys()` (Phase 06 PC-004) returning all non-empty keys (`image` + any set `thumbnail_*`), then have all 6 sweep commands + `soft_delete_user_ads` + `consent_hard_delete` call it — collapsing 7 `values_list` sites into 1 model method. Pass each returned key to `delete_photo` (with the ME-001 containment check in place). Add reference-counting for shared seed thumbnails (seed keys are excluded by `sweep_orphaned_media`'s `seed/` directory filter, not by count). Medium effort; depends on ME-001 (containment check must be in `delete_photo` first).
...
- The orphan sweep uses `os.remove` directly (not `delete_photo`) — routing through `delete_photo` is DEFERRED to post-ENT-001 relocation (use the moved `apps/media/services/filesystem.py` location, not the current bot-path `delete_photo`); see NEW-ME-007.
...
| **Cross-phase duplication** | ME-003's non-atomicity claim overlaps Phase 03 (DB-004). DB-004's outbox recommendation is **formally superseded** by ME-003's sweep-fix approach (see ME-003 cross-reference note). Do not implement `MediaPendingDelete` or `AdvisoryLockId.MEDIA_REAPER = 13`. |
...
1. **ME-001**: Add `os.path.realpath` containment check in `delete_photo` (at the ENT-001-moved location `apps/media/services/filesystem.py`, not the current bot-path `media.py`); add `RegexValidator`/`CheckConstraint` on `AdImage.image` and all three `thumbnail_*` fields. Apply containment to `AdImage.save()`. **Routing `sweep_orphaned_media`'s `os.remove` through `delete_photo` is DEFERRED** to post-ENT-001 relocation — see NEW-ME-007.
2. **NEW-ME-007**: Add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` (ID 103 retained for orphan sweep; `CATALOG_LOAD` reassigned to 104 — see Cross-Phase Reconciliation note at top of file); schedule `sweep_orphaned_media` in hourly scheduler; add lock-structure test; add CI assertion in `test_advisory_lock_ids.py` that every `AdvisoryLockId.*` call-site reference is defined in the enum.
...
2. **ME-003**: After NEW-ME-007 is fixed, escalate `delete_photo`'s exhausted-retry path from `logger.error` to a structured metric/event (e.g. `AnalyticsEvent`). The now-functional `sweep_orphaned_media` reconciliation backstop covers automatic recovery — no outbox table needed. **Phase 03 DB-004's outbox recommendation (`MediaPendingDelete` + `AdvisoryLockId.MEDIA_REAPER = 13`) is formally superseded** — see ME-003 cross-reference note above.
3. **ME-002**: After ME-001 and NEW-ME-007 are fixed, implement `AdImage.storage_keys()` (Phase 06 PC-004) and have all 6 sweeps + `soft_delete_user_ads` + `consent_hard_delete` call it — collapsing 7 `values_list` sites into 1 model method. Delete each returned key via `delete_photo`. Add reference-counting for shared seed thumbnails. Add regression test asserting thumbnail cleanup after sweep.
...
- **Leverages existing infrastructure**: no new model, no migration, no new process. The hourly scheduler already runs 8 sweep commands; adding `sweep_orphaned_media` is a one-line change.
  - **Lower risk**: a periodic sweep is idempotent and bounded — it deletes only files with no DB reference, so it can never orphan or delete live data (assuming ME-001's containment check is also applied, per NEW-ME-007's deferred recommendation to route through `delete_photo` post-ENT-001 relocation).
...
| `src/backend/apps/media/management/commands/sweep_orphaned_media.py` | (Secondary, post-ME-001 **and** post-ENT-001 relocation) Route `os.remove` (line 112) through `delete_photo` (from `apps/media/services/filesystem.py`) for containment check |
