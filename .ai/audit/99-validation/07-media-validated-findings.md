...
3. [Recommended] **Bot-side per-Telegram-user upload throttling** (see *Rejected alternative* below). Create `src/telegram_bot/services/rate_limit.py` mirroring the existing `apps/search/services/rate_limit.py` pattern (Django `cache.add` + `cache.incr` atomic increment, 60s TTL) but keyed by `upload_rl:{user_id}` with a configurable limit (e.g. 10 uploads per 60s). In `process_photos` (`src/telegram_bot/handlers/ad_create.py:395-402`), before calling `save_photo`, invoke `check_photo_upload_rate(message.from_user.id)`; if it returns `False`, answer 'Upload limit reached. Please wait before sending more photos.' and `return` before the disk write. This is the correct grain because the bot is the **sole write path** for media (sellers post ads via the bot per AGENTS.md) and `message.from_user.id` (available at `ad_create.py:56`) is the only stable per-seller identity the bot can observe. The bot process already runs `django.setup()` + shared ORM, so `django.core.cache` (shared Redis/memcached per Django settings) is available with no new plumbing.
   - **Rejected alternative — nginx `/media` rate zone:** Considered and rejected. `location /media/` (nginx.conf:61-67) is a **read-only** serving path that proxies to the web gunicorn WSGI to deliver images to *browsers* (buyers); media never enters the system through nginx, so a `limit_req_zone` there would throttle downloads, not uploads. A zone on the bot's webhook ingress would throttle *all* inbound bot traffic indiscriminately rather than per-user photo uploads, and IP-based limiting is ineffective because Telegram masks the real client IP (all webhook requests arrive from Telegram's edge, not the seller). The existing `apps/search/services/rate_limit.py` confirms the project's established pattern is Django-cache-based per-identity throttling, not nginx zones, for application-layer rate control. The bot-side approach also composes correctly with the count guard in item 1: the rate limiter bounds *bursts*, while the count guard bounds *per-ad accumulation*.
