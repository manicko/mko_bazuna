...
# Audit Findings — Database & Concurrency Consistency (Validated)
...
**Validation result:** All four findings are **VALIDATED** — each was verified against the actual source code, test suite, and project patterns. One minor precision note: DB-001's evidence cites `record_event` as a potential raiser, but `record_event` has a defensive `try/except Exception` (analytics.py:45-60) that suppresses all errors; the actual raiser at line 268 is `TrustCalculator().calculate_and_save`. The core claim is correct.
...
|----|-------|----------|--------|----------|
| DB-001 | `submit_ad` (bot path) commits ad + images before auto-moderation | CRITICAL | Validated | Transaction atomicity |
| DB-002 | Sync Redis cache I/O blocks aiogram event loop in 3 async call sites | HIGH | Validated | Async/sync bridge |
| DB-003 | `ban_user_for_ad` and `bulk_ban_users` write ban and audit log non-atomically | MEDIUM | Validated | Transaction atomicity |
| DB-004 | Login handler splits token-claim and user-creation into two transactions | MEDIUM | Validated | Cross-process consistency |
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** Verified against source code. In `submission.py:158-179`, the `with transaction.atomic():` block ends at line 179 (`ad.transition_to(AdStatus.ON_MODERATION)`) with a blank line at 180. `auto_moderate(ad)` at line 186 is at the same indentation level as the `with` statement, confirming it executes **outside** the atomic block. In `auto_moderation.py`, both `_fail_moderation` (line 236) and `_pass_moderation` (line 253) open their own `transaction.atomic()` blocks. In the bot path (`ad_create.py:813`), `submit_ad` is called via `sync_to_async` with no enclosing `transaction.atomic()`, so `auto_moderate`'s inner transaction is a real commit boundary. In the web edit path (`edit.py:125-178`), `submit_ad` is called inside the outer `transaction.atomic()`, making `auto_moderate`'s inner blocks SAVEPOINTs. The impact scenario is real: `TrustCalculator().calculate_and_save(ad.user)` at `auto_moderation.py:268` can raise (no defensive try/except), and `set_published` at `moderation_log.py:225` can raise `MaxAdsExceeded` (caught by `auto_moderate`) or other errors. Minor precision note: the finding text cites `record_event` as a potential raiser, but `record_event` (analytics.py:45-60) has a broad `try/except Exception` that suppresses all errors — it will not propagate. The line 268 reference is actually `TrustCalculator().calculate_and_save`, which is the real raiser. Core claim remains correct.
> - **See also:** DB-003 (shared root cause: transaction-scope-too-narrow)
...
| **Impact** | If `auto_moderate` fails partway (e.g., `TrustCalculator().calculate_and_save` at line 268 raises, or `set_published` raises an error other than `MaxAdsExceeded`), the ad is already committed in `ON_MODERATION` with images persisted, but the status transition to `PUBLISHED`/`ON_MODERATION_FAILED`, the `ModeratorActionLog`, and `AnalyticsEvent` records are lost. The ad is left in limbo — `ON_MODERATION` with no moderation outcome — requiring manual intervention. In the web edit path this is mitigated because `edit.py:125` wraps `submit_ad` in an outer `transaction.atomic()`, making `auto_moderate`'s inner block a SAVEPOINT. But the bot path has no such outer transaction. |
| **Root Cause** | `auto_moderate` is called after the `with transaction.atomic():` block closes (line 181-186), and `auto_moderation._pass_moderation`/`_fail_moderation` each open their own transaction rather than participating in the caller's transaction. |
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** All three call sites verified against source code. `ad_create.py:675` calls `check_upload_rate_limit(user_id)` inside `async def process_photos` (decorated `@router.message(AdCreateForm.photos)`, line 623-624) — no `sync_to_async` wrapper. `contact.py:118` calls `check_contact_start_rate_limit(message.from_user.id)` inside `async def handle_contact_us_start` (line 104) — no `sync_to_async` wrapper. `update_id_dedup.py:58` calls `cache.add(...)` inside `async def __call__` (line 49) — no `sync_to_async` wrapper, and `sync_to_async` is not even imported in this module. Both the rate-limit functions (`telegram_bot/services/rate_limit.py:30` and `:81`) are plain `def` (sync). `ad_create.py` imports `sync_to_async` (line 21) and `contact.py` imports `sync_to_async` (line 17) but neither uses it to wrap the rate-limit call. A grep for `sync_to_async(check_upload_rate_limit)`, `sync_to_async(check_contact_start_rate_limit)`, and `sync_to_async(cache.add)` across `src/telegram_bot/` returns zero matches. Production uses Redis-backed cache (django-redis) per the project's runtime assumption, confirming network I/O on the event loop. Test impact: `test_update_id_dedup.py` monkeypatches `dedup_module.cache` as a `MagicMock` — the `sync_to_async` fix would require the mock's `.add` method to be thread-safe; existing tests should continue to pass since MagicMock is thread-safe for simple use. `test_ad_create.py:241,264` monkeypatches `check_upload_rate_limit` with sync lambdas — `sync_to_async(lambda)` wrapping would work since the lambda is captured at call time after monkeypatch. `test_contact_us.py` calls `check_contact_start_rate_limit` directly (testing the sync function itself), which is unaffected by the handler-level fix.
> - **See also:** DB-001 (structurally independent)
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** Both functions verified against `admin_actions.py`. `ban_user_for_ad` (lines 66-87): `user.save(update_fields=["is_banned"])` at line 78 commits immediately, then `log_ban_account(...)` at lines 80-84 creates a `ModeratorActionLog` via `ModeratorActionLog.objects.create()` (moderation_log.py:135) in a separate implicit transaction. No `transaction.atomic()` wraps the pair. `bulk_ban_users` (lines 201-226): `log_ban_account` in a loop (lines 218-222) creates audit log entries, then `User.objects.filter(id__in=user_ids).update(is_banned=True)` at line 225 is a separate implicit transaction. No `transaction.atomic()` wraps the sequence. Sibling functions confirm the pattern: `soft_delete_ad` (line 105), `bulk_approve` (line 131), `bulk_reject` (line 177), `bulk_delete` (line 242) all use `with transaction.atomic():`. The only test for `bulk_ban_users` (`test_bulk_ban_users_not_locked` at test_admin_actions.py:242-245) asserts only `assert "select_for_update" not in src` — it does not assert `transaction.atomic` presence. There is no test for `ban_user_for_ad` at all. Both functions are called from the web process (`review.py:163` and `admin.py:140`), not the bot, but the `User` model is shared with the bot login flow, creating cross-process consistency risk.
> - **See also:** DB-001 (shared root cause)
...
**Evidence — sibling functions use `transaction.atomic()`** *(supports: "inconsistent pattern with siblings")*:
- `soft_delete_ad`: line 105 `with transaction.atomic():`
- `bulk_approve`: line 131 `with transaction.atomic():`
- `bulk_reject`: line 177 `with transaction.atomic():`
- `bulk_delete`: line 242 `with transaction.atomic():`
...
> **Validation Note:**
> - **Action:** validated
> - **Detail:** Verified against `login.py:177-213`. The `_handle` function (decorated `@sync_to_async`, line 177) contains two separate `with transaction.atomic():` blocks: the first at line 184 wraps `_claim_login_token(token_hash, telegram_id, now)` (line 185), which uses raw SQL `UPDATE ... RETURNING` via `connection.cursor()` (login.py:137-155) to consume the login token. The second at line 192 wraps `User.objects.get_or_create(...)` (lines 193-202). The first transaction commits before the second begins. If `get_or_create` raises any exception other than `IntegrityError` (caught at line 203), the token has already been consumed and permanently committed — the user cannot complete login and must request a new token. The docstring at line 166 states "Atomically claim a login token, then get or create the user" which contradicts the two-transaction implementation. `_claim_login_token` itself uses `UPDATE ... RETURNING` (single atomic SQL statement), so the claim itself is race-free, but the two-phase commit is the gap. The handler is `async def handle_login_orm` (line 158), wrapped with `@sync_to_async` internally — no async/sync bridge issue here.
> - **See also:** None
...
- **Merge candidates:** DB-001 and DB-003 both stem from transaction-scope-too-narrow patterns. They address different code paths (ad submission vs. user banning) with different severities (CRITICAL vs. MEDIUM), so they remain separate findings — validated as appropriate.
- **Conflicting evidence:** None. All findings verified against the same source tree with consistent results.
- **Dependency chains:** None. DB-002 (sync cache I/O) is structurally independent of the transaction findings. DB-001 and DB-003 share a root cause but touch different code paths. The fix-ordering recommendation (DB-001 → DB-002 → DB-003 → DB-004) is sensible prioritization (CRITICAL before MEDIUM, HIGH before MEDIUM) but not a hard dependency: `submit_ad` → `auto_moderate` → `_pass_moderation` does not call `ban_user_for_ad`; no current code path chain exists from `ban_user_for_ad` through `auto_moderate`.
...
| DB-002 | Low | Yes | No test exercises sync cache calls from async context; add a test that detects blocking cache I/O in async handlers (e.g., mock cache backend to record thread-blocking). `test_update_id_dedup.py` monkeypatches `cache` as `MagicMock` — verify `sync_to_async(cache.add)` works with mocked cache in threads. `test_ad_create.py` monkeypatches `check_upload_rate_limit` with sync lambdas — verify `sync_to_async` wrapping still resolves the patched function. |
| DB-003 | Low | Yes | `test_bulk_ban_users_not_locked` only asserts `select_for_update` absence; add assertion that `transaction.atomic` is present. No test exists for `ban_user_for_ad` — add one verifying atomicity of ban + audit log. |
| DB-004 | Low | Yes | No test simulates `get_or_create` failure after token claim; add test that patches `get_or_create` to raise (non-IntegrityError) and asserts token is unclaimed. Verify `_claim_login_token` raw SQL `UPDATE ... RETURNING` rollback works correctly when nested in the caller's transaction. |
...
| Finding | Code Verified | Tests Verified | Patterns Verified |
|---------|---------------|----------------|-------------------|
| DB-001 | `submission.py:158-186`, `auto_moderation.py:94,236,253,268`, `ad_create.py:813`, `edit.py:125-178` | No test covers `submit_ad`+`auto_moderate` transaction boundary | Django SAVEPOINT nesting confirmed; sibling `edit.py` path verified as mitigated |
| DB-002 | `ad_create.py:675`, `contact.py:118`, `update_id_dedup.py:58`, `rate_limit.py:30,52,56,81,104`, imports in `ad_create.py:21`, `contact.py:17` (sync_to_async imported but not used for rate-limit calls) | `test_ad_create.py:241,264` (lambda mocks), `test_update_id_dedup.py:29-73` (MagicMock cache), `test_contact_us.py:111,118` (direct sync calls) | Async context confirmed: `async def process_photos` (line 624), `async def handle_contact_us_start` (line 104), `async def __call__` (line 49). `sync_to_async` not imported in `update_id_dedup.py` |
| DB-003 | `admin_actions.py:66-87,201-226`, `moderation_log.py:121-141` (log_ban_account creates ModeratorActionLog), sibling functions at lines 105,131,177,242 | `test_admin_actions.py:242-245` (only checks `select_for_update` absence), no test for `ban_user_for_ad` | Sibling pattern (`transaction.atomic()`) confirmed at 4 call sites; DB-003 functions are only callers missing it |
| DB-004 | `login.py:122-155` (_claim_login_token raw SQL UPDATE...RETURNING), `login.py:177-213` (two `transaction.atomic()` blocks), `login.py:166` (docstring) | No test simulates `get_or_create` failure after token claim | Raw SQL `UPDATE ... RETURNING` is single-statement atomic; `IntegrityError` catch at line 203 is narrow; non-IntegrityError exceptions leave token consumed |

### Warnings

- **DB-001 behavioral change (rollout):** Moving `auto_moderate` inside `submit_ad`'s `transaction.atomic()` changes rollback semantics: an unhandled exception in moderation (e.g., `TrustCalculator` failure) will now roll back the entire ad submission, leaving no `ON_MODERATION` record. This is the desired atomicity guarantee but changes behavior from "ad persists in ON_MODERATION" to "ad is not persisted at all." The web edit path will have 3+ levels of SAVEPOINT nesting (edit.py outer → submit_ad → auto_moderation → set_published) — Django handles this correctly but monitoring should watch for SAVEPOINT overhead.
- **DB-001 precision note:** The finding text cites `record_event` as a potential raiser at line 268, but `record_event` (analytics.py:45-60) has a broad `try/except Exception` that suppresses all errors. Line 268 is actually `TrustCalculator().calculate_and_save(ad.user)`, which is the real raiser with no defensive catch in this call chain.
- **DB-002 test impact:** `test_update_id_dedup.py` monkeypatches `cache` as a `MagicMock` and asserts `mock_cache.add.assert_called_once_with(...)`. After wrapping in `sync_to_async`, the mock's `.add` runs in a thread executor — tests should pass since MagicMock is thread-safe for simple calls, but the assertion timing may need adjustment.
- **DB-004 raw SQL caveat:** `_claim_login_token` uses `connection.cursor()` with raw SQL `UPDATE ... RETURNING` inside `transaction.atomic()`. When merged into a single transaction with `get_or_create`, the `UPDATE` row lock will persist until the transaction exits — this is correct behavior but means the `LoginToken` row is locked for the duration of `get_or_create`, which could cause lock contention under high concurrency if user creation is slow.
