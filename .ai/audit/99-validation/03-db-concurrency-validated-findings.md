...
> **Validation Correction (post-Validator R2):** The Validator identified four recommendation bugs; all are corrected in-text below.
> - **DB-002 FieldError:** The recommendation prescribed `User.objects.select_for_update(of='user')` — verified against Django 5.2.16 `compiler.py:1471-1509` that `User` has no `user` relation, so `of='user'` raises `FieldError`. **Fixed** to bare `User.objects.select_for_update().get(pk=ad.user_id)` (equivalently `of='self'`).
> - **DB-003 FieldError:** The same `FieldError` defect was carried in DB-003's `select_for_update(of='ad')` — the `Ad` model has no `ad` field/relation. **Fixed** to bare `Ad.objects.select_for_update().get(...)`.
> - **DB-002 centralization:** Research confirmed all three publish writers (bot `auto_moderate`, web-edit, web-review) converge on `moderation_log.set_published` — so the User-row lock + authoritative re-count is centralized there (one edit, all writers), eliminating the partial-deploy atomicity window. The "ship bot+web together" constraint is relaxed to a single `set_published` edit.
> - **DB-004 ↔ ME-003 resolution:** DB-004's pending-delete outbox (`MediaPendingDelete` model + `reap_pending_deletes` + `AdvisoryLockId.MEDIA_REAPER=13`) is **superseded** by Phase 07 ME-003/NEW-ME-007's reconciliation-sweep approach (fix + schedule `sweep_orphaned_media`; enum `SWEEP_ORPHANED_MEDIA=103` + hourly scheduler entry). No daemon exists to poll an outbox; zero outbox/dead-letter precedent. `MediaPendingDelete` and `AdvisoryLockId.MEDIA_REAPER=13` are **NOT** allocated. Root-cause analysis (non-atomic DB-then-delete) is retained; remediation redirects to the sweep.
> - **DB-001 reconnect (B):** The `dp.run_polling` reconnect wrapper is safe-but-redundant under Docker `restart: unless-stopped` (docker-compose.yml:188). **Softened** to optional/deferred — ship the per-update `close_old_connections` middleware first; evaluate the reconnect wrapper against a measured DB-blip recovery SLO.
...
**Recommendation:** Add an aiogram middleware that calls `close_old_connections()` at the end of each update's sync ORM work (or scope the sync bridge to open/close per invocation). Effort: small. Priority: mandatory. **Reconnect wrapper (DEFERRED):** Add DB-unavailability detection + reconnect around `dp.run_polling` — operational hardening, not a correctness requirement. Safe-but-redundant under Docker `restart: unless-stopped` (docker-compose.yml:188) for process-level recovery; ship the middleware first, then evaluate the reconnect wrapper against a measured DB-blip recovery SLO.
...
**Recommendation:** [SELECTED approach: B -- reconciliation sweep (ME-003/NEW-ME-007); outbox APPROACH SUPERSEDED] **Superseded per DB-004 ↔ ME-003 cross-phase resolution** (see Validation Correction): Phase 07 ME-003/NEW-ME-007 selected the reconciliation-sweep approach — fix and schedule the existing `sweep_orphaned_media` command (enum `SWEEP_ORPHANED_MEDIA=103` + hourly scheduler entry) rather than building a pending-delete outbox. No daemon exists to poll an outbox; the codebase has zero outbox/dead-letter precedent (project rule #7); `sweep_orphaned_media` already implements the reconciliation algorithm (collects all 4 AdImage fields, diffs store vs DB). `AdvisoryLockId.MEDIA_REAPER=13` and `MediaPendingDelete` are **NOT** allocated. The root-cause analysis (non-atomic DB-then-delete crash orphans files) is retained; the remaining recommendation text below is **formally retired** in favor of the sweep. Rationale from codebase research: (1) All SIX media-deleting sweeps follow the identical pattern: `transaction.atomic()` + `advisory_lock(N)` to collect `image` keys then `queryset.delete()` then `delete_photo` outside tx (delete_sweep.py:43-79, sweep_drafts.py:42-77, purge_deleted_ads.py:44-79, purge_failed_ads.py:42-77, purge_rejected_ads.py:43-79, consent_hard_delete.py:45-90) -- the codebase's established management-command + advisory-lock architecture (AdvisoryLockId enum: IDs 1-12 for jobs, 100-111 for services, advisory_lock.py:36-37) is the natural template for the reaper; (2) `delete_photo` (media.py:85-123) already has 3-attempt exponential backoff and swallows all exceptions (never raises) -- the per-file error containment is already met; the gap is purely the crash-between-commit-and-delete window with no retry path; (3) `sweep_orphaned_media.py` exists as a broken filesystem-level backstop (`AdvisoryLockId.SWEEP_ORPHANED_MEDIA` at line 87 is NOT defined in the enum per NEW-ME-007, and it is unscheduled in entrypoint-scheduler.sh:28-37 -- providing zero working crash recovery); (4) Approach A (delete files BEFORE DB) is REJECTED because a DB rollback after file deletion leaves DB rows pointing to already-deleted files -- a worse data-integrity outcome than orphaned files (which are at most a storage leak). [RETIRED — superseded by the reconciliation sweep per DB-004 ↔ ME-003 resolution; see Validation Correction. The pending-delete outbox model (`MediaPendingDelete`) and `AdvisoryLockId.MEDIA_REAPER=13` are NOT allocated.] **Concrete changes (redirected to sweep -- outbox RETIRED):** (1) Add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` in `core/enums.py` (after `BACKFILL_THUMBNAILS = 102`) -- the sweep's own docstring (`sweep_orphaned_media.py:13`) already documents "Uses advisory lock 103"; (2) add `sweep_orphaned_media` to `entrypoint-scheduler.sh` `hourly_commands`; (3) add lock-structure test entries to `test_sweep_lock_structure.py` (`SWEEP_COMMANDS` + `_LOCK_TARGET_MODULES`); (4) route `sweep_orphaned_media.py:112` raw `os.remove` through `delete_photo` (`media.py:85-123`, already retry-capable + error-swallowing). [RETIRED -- the following original Concrete-changes items (2)-(7) described the pending-delete outbox (`MediaPendingDelete` model + `reap_pending_deletes` command + `AdvisoryLockId.MEDIA_REAPER=13`); all superseded by the reconciliation sweep (see Validation Correction). `MediaPendingDelete` and `AdvisoryLockId.MEDIA_REAPER=13` are NOT allocated. No migration needed -- no new model.] Sweep rollout: enum fix (103) + scheduler slot + test updates co-deploy (`entrypoint-scheduler.sh` runs `subprocess.run([...,'manage.py', cmd], check=False)` hourly -- lines 28-37 -- the sweep is a 12th sibling to the existing 11 commands). No circular dependencies. Effort: small. Priority: advisory.
...
- **DB-001 (HIGH):** Add `close_old_connections()` per bot update (aiogram middleware). The reconnect wrapper around `dp.run_polling` is **deferred** — safe-but-redundant under Docker `restart: unless-stopped` (see Validation Correction); ship the middleware first, evaluate reconnect against measured DB-blip recovery SLO.
- **DB-002 (HIGH):** Centralize the `max_ads_per_user` count check inside `set_published`'s existing `transaction.atomic()` under `User.objects.select_for_update().get(pk=ad.user_id)` — one edit covers bot (`auto_moderate`), web-edit, and web-review (all converge on `set_published`), eliminating the partial-deploy atomicity window. Rejects DB-level constraint approach (dynamic cap cannot be a static constraint).
- **DB-003 (MEDIUM):** `select_for_update()` at fetch sites in `review.py` and `edit.py`, plus `self.refresh_from_db()` in `Ad.transition_to` itself. Rejects DB-side rowcount precondition approach (would bypass `post_save` signals and depart from `self.save()` pattern).

## Advisory Recommendations

- **DB-004 (MEDIUM):** Adopt ME-003/NEW-ME-007 reconciliation sweep — add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` + add `sweep_orphaned_media` to the hourly scheduler + route its `os.remove` through `delete_photo`. The pending-delete outbox approach is **superseded** (see Validation Correction); `MediaPendingDelete` and `AdvisoryLockId.MEDIA_REAPER=13` are **NOT** allocated.
...
- **DB-001 (mandatory, HIGH):** Adding an aiogram `close_old_connections` middleware is purely additive (safe to introduce incrementally, before DB-002). The reconnect wrapper around `run_polling` is **deferred** to operational SLO evaluation (see Validation Correction) — Docker `restart: unless-stopped` (docker-compose.yml:188) already provides process-level recovery. The middleware does not interact with advisory locks, so no circular dependency or ordering conflict.
- **DB-002 (mandatory, HIGH):** Centralizing the count check inside `set_published`'s existing `transaction.atomic()` with `User.objects.select_for_update().get(pk=ad.user_id)` introduces a row lock on the User row during publish. Under PgBouncer transaction-mode (confirmed: base.py:186 `CONN_MAX_AGE=0`, `:188` `prepare_threshold: None`, docker-compose.prod.yml:100), `SELECT FOR UPDATE` is fine; the lock is held for the tx duration and released at commit. Because all three writers (bot `auto_moderate`, web-edit, web-review) converge on `set_published`, a single deploy covers all paths — no cross-process ship-together requirement. No circular dependency.
- **DB-003 (mandatory, MEDIUM):** `select_for_update()` at fetch sites + `self.refresh_from_db()` in `transition_to` adds row locks on the Ad row. The moderation fetch + transition must be wrapped in `transaction.atomic()` (the inner `set_published`/`set_rejected` txs become savepoints). Lock contention with hourly sweeps (which use bulk `update()`/`delete()` acquiring PK-order row locks) is safe -- single-row lock vs bulk lock = wait, not deadlock. Sweeps already use advisory locks (1-4, 6, 7, 11); moderation does not acquire one, but the PK-ordered bulk locks prevent circular waits. No unsafe insertion points.
- **DB-004 (advisory, MEDIUM):** Resolved to ME-003/NEW-ME-007's reconciliation sweep (scheduler-only, no migration). Rollout: add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` (enums.py) + add `sweep_orphaned_media` to `entrypoint-scheduler.sh` `hourly_commands` + test entries must co-deploy (enum-without-scheduler = silent no-op; scheduler-without-enum = hourly `AttributeError`). The pending-delete outbox (`MediaPendingDelete` model + `reap_pending_deletes` + `AdvisoryLockId.MEDIA_REAPER=13`) is **superseded** (see Validation Correction); no migration required. No circular dependency.
- **Dependency ordering:** DB-001 must ship before DB-002 (lock on a healthy, refreshable connection). DB-003 and DB-004 (sweep) are independent of DB-001/DB-002. No circular dependencies.

### Execution Validation

| Finding | Targets still exist? | Static verified? | Ready for execution |
|---------|----------------------|------------------|---------------------|
| DB-001 | Yes — main.py:43,65; base.py:186; login.py:151,159; contact.py:122,143; ad_create.py:870; permissions.py:150; conftest.py:164-165 | Yes (grep + file reads) | Yes — small (middleware only; reconnect deferred) |
...
| DB-004 | Yes — delete_sweep.py:43,72,77-78; sweep_drafts.py:42,70,75-76; consent_hard_delete.py:45,84,89-90; media.py:85 | Yes | Yes — small (sweep fix + scheduling) |
...
5. **DB-004 partial mitigation already present:** `delete_photo` (media.py:85-123) already implements 3-attempt exponential backoff and swallows errors (never raises), so a single failing file does not abort the sweep loop. The finding's "at minimum" recommendation is partially met; the remaining gap (crash between DB commit and file-deletion orphans files with no recovery) is remediated by ME-003/NEW-ME-007's reconciliation sweep, which supersedes the original outbox recommendation (see Validation Correction).

6. **DB-002 cross-process gap:** The web `approve_ad` path (`review.py:64-65` → `admin_actions.approve_ad:40` → `set_published`) performs NO `max_ads_per_user` count check at all (unlike the bot path which calls `auto_moderate`). This means a moderator manual-approve can exceed the cap even without a race — a stricter finding than the TOCTOU alone. Centralizing the lock + authoritative re-count in `set_published` (DB-002's recommendation) closes this gap in the same single edit, since `approve_ad` calls `set_published` directly.

### Required Fixes

1. **DB-001 (mandatory):** Register an aiogram update-scoped middleware that calls `close_old_connections()` after each bot update's sync ORM work (reuse the `connection_created` signal pattern proven in `conftest.py:193-213`); deploy DB-001 **before** DB-002 so the row lock is acquired on a healthy, refreshable connection. The reconnect wrapper around `dp.run_polling` is **deferred** — safe-but-redundant under Docker `restart: unless-stopped` (docker-compose.yml:188); ship the middleware first, evaluate reconnect against measured DB-blip recovery SLO.
2. **DB-002 (mandatory):** In `set_published` (`moderation_log.py:208-224`), add `User.objects.select_for_update().get(pk=ad.user_id)` at the top of its existing `transaction.atomic()` (line 218), then re-count (`Ad.objects.filter(user_id=ad.user_id, status__in=[PUBLISHED, ON_MODERATION]).count()`) and raise a typed `MaxAdsExceeded` if `>= max_ads` before `transition_to(PUBLISHED)`. This single edit covers all three writers (bot `auto_moderate`, web-edit, web-review — all call `set_published`), and closes the web-review path's missing count check. Rejects DB-level constraint (dynamic cap).
3. **DB-003 (mandatory):** In `review.py` and `edit.py`, add `select_for_update()` to Ad fetches inside the `transaction.atomic()` that covers the transition. Insert `self.refresh_from_db()` at the top of `Ad.transition_to` (ads/models.py:349) as a single chokepoint. Rejects DB-side rowcount precondition (would bypass `post_save` signals).

### Advisory Recommendations

1. **DB-004:** Adopt ME-003/NEW-ME-007 reconciliation sweep — do NOT build the `MediaPendingDelete` outbox or `reap_pending_deletes` command. (a) Add `SWEEP_ORPHANED_MEDIA = 103` to `AdvisoryLockId` (core/enums.py, after `BACKFILL_THUMBNAILS = 102`); (b) add `sweep_orphaned_media` to `entrypoint-scheduler.sh` `hourly_commands`; (c) add lock-structure test entries to `test_sweep_lock_structure.py` (`SWEEP_COMMANDS` + `_LOCK_TARGET_MODULES`); (d) route `sweep_orphaned_media.py:112` raw `os.remove` through `delete_photo` (media.py:85-123). The sweep already collects all 4 AdImage fields and diffs store-vs-DB, so it subsumes both crash-orphaned-file recovery AND the thumbnail-orphan gap (ME-002). No migration; `AdvisoryLockId.MEDIA_REAPER=13` is **NOT** allocated.
2. **DB-002 (web path):** The web-review `approve_ad` path currently bypasses the cap entirely (no count check). Centralizing the lock + re-count in `set_published` (DB-002's main recommendation) closes this automatically, since `approve_ad` calls `set_published` — no separate web-path edit is needed.
