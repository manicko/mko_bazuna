...
**Recommendation:** [BEST-PRACTICE] Persist processed `update_id`s (dedup table or Redis SET) and skip re-delivered updates in an update-level middleware; or make `create_draft_ad` idempotent keyed on the originating `message.message_id`/`update_id` (e.g. `get_or_create` on `(user, telegram_update_id)` for in-DRAFT state). At minimum, guard side-effects (photo writes, translation calls) so a re-delivered update cannot double-create. Effort: medium. Priority: recommended.
...
- **EXT-005 (advisory, medium):** Idempotent `create_draft_ad` (via `get_or_create` keyed on `update_id`) + optional update-id-dedup middleware. **Rollout coupling:** the idempotency key must be stable across a re-delivered update (Telegram's `update_id` is stable per-update); a `get_or_create` on `(user_id, telegram_update_id)` requires the field to exist on `Ad` (schema change → migration). If the column doesn't exist, this is a migration prerequisite (project rule #13: migrations for schema changes). The dedup-middleware path (persist `update_id` in a table/Redis) is an alternative with no schema change. Either way, safe once the chosen anchor exists. No unsafe insertion points.
