...
> **Validation Correction (Planner-edited):** The following corrections were applied to this findings document per the Validator report (`09-external-validator.md`) and Research report (`EXT-001-research.md`), after re-verification of the working tree and the installed `deep_translator` 1.9.1 exception hierarchy:
>
> 1. **EXT-001 except-narrowing (mandatory prerequisite):** The narrowed-except clause now explicitly includes the `deep_translator.exceptions` family — `(TimeoutError, RequestException, deep_translator.exceptions.TooManyRequests, deep_translator.exceptions.RequestError, deep_translator.exceptions.TranslationNotFound)`. Research VERIFIED that `TooManyRequests`, `RequestError`, and `TranslationNotFound` subclass only `Exception` (NOT `requests.exceptions.RequestException`), so a bare `(TimeoutError, RequestException)` narrowing would let Google 429/non-200/no-element propagate into `translate_all_languages`' `asyncio.gather` and crash ad creation. The family is split into retryable (`TooManyRequests` / 429, `RequestError` / non-200) vs fallback-only (`TranslationNotFound` / no-element → same input returns same response, never retry). This narrowing is promoted from Advisory to a **hard gating prerequisite** in Required Fixes — it MUST ship before the httpx migration stage.
> 2. **EXT-001 Warning #4 → EXT-005 (no migration for chosen approach):** Warning #4 stated the chosen idempotency approach needs a schema migration. The Validator confirmed the chosen `UpdateIdDedupMiddleware` (Approach A) persists `update_id` in the Django-cache/Redis backing store, NOT on the `Ad` model — **no migration required**. The migration prerequisite applies only to the rejected `get_or_create`-on-`update_id` alternative. Warning #4 is revised accordingly.
> 3. **EXT-002 asyncio.gather `return_exceptions=True`:** Research confirmed the `asyncio.gather` at `immediate_alerts.py:195` has NO `return_exceptions=True` — a single failed `_send` propagates out of `gather`, cancelling all in-flight sibling sends in the thread's batch. This is added to the EXT-002 recommendation.
> 4. **EXT-002 module-level `Bot` (infeasible):** Research VERIFIED a process-global shared `Bot` is structurally infeasible (`aiohttp.ClientSession` binds to the event loop at creation; `asyncio.run` creates a fresh loop per thread). The recommendation is corrected from "reuse a single module-level Bot per worker thread" to "one `Bot` per thread/loop, reused across the batch, closed once" — matching the existing pattern at `send_alerts.py:144,182`.
...
**Recommendation:** [BEST-PRACTICE] Add bounded retry with capped exponential backoff (1–2 attempts) at the `translate_text` layer; collapse the redundant `asyncio.to_thread`→executor double-hop into a single concurrency scope bounded by a `Semaphore` so saturated throttling degrades to fallback instead of stalling; **first** narrow the `except` to `(TimeoutError, RequestException, deep_translator.exceptions.TooManyRequests, deep_translator.exceptions.RequestError, deep_translator.exceptions.TranslationNotFound)` — a bare `(TimeoutError, RequestException)` narrowing is unsafe because the deep_translator family subclasses only `Exception` (not `requests.exceptions.RequestException`), so it would let Google 429/non-200/no-element propagate into `translate_all_languages`' `asyncio.gather` (`ad_create.py:1270-1274`) and crash ad creation instead of falling back: **retry** `TimeoutError`, `RequestException`, `deep_translator.exceptions.TooManyRequests` (429), and `deep_translator.exceptions.RequestError` (non-200); **fall back only** on `TranslationNotFound` (no-element → same input returns same empty response, never retry); any exception outside this set (e.g. `AttributeError`/`KeyError` from a malformed translator response) propagates so programming errors surface; migrate off deep-translator's un-timeout'd `requests` to `httpx` with an explicit per-request timeout + cancel scope so a timed-out call is actually cancelled rather than leaving an orphaned worker. Effort: medium. Priority: recommended. **Prerequisite:** the except-narrowing **must** include the `deep_translator.exceptions` family — it is a hard gating prerequisite that must ship before the httpx migration stage; narrowing to `(TimeoutError, RequestException)` alone crashes ad creation on the first Google 429/no-element.
...
**Recommendation:** [BEST-PRACTICE] Construct **one `Bot` per thread/loop** (one `aiohttp` session), reused across the batch and closed once after the batch — NOT a process-global shared `Bot`, which is infeasible because `aiohttp.ClientSession` binds to the event loop running at creation time (`asyncio.run` creates a fresh loop per thread), so a module-level `Bot` shared across daemon threads fails structurally (this mirrors the existing correct pattern at `send_alerts.py:144,182`); add `return_exceptions=True` to the `asyncio.gather` at `immediate_alerts.py:195` so a single failed `_send` does not cancel all in-flight sibling sends — classify results (retry transient: `TelegramRetryAfter` honouring `retry_after` + 5xx/network with backoff; dead-letter permanent: `TelegramForbiddenError`/`TelegramBadRequest`); bound the number of concurrent delivery threads globally (process-level semaphore / bounded `ThreadPoolExecutor`) so a publish burst cannot spawn unbounded threads; narrow the bare `except Exception` at `immediate_alerts.py:164-168` to the `AiogramError` family so programming errors propagate. Effort: medium. Priority: recommended.
...
| EXT-005 | Yes — `main.py:39,43-58,65`; `ad_create.py:870-875,1270-1274`; `login.py:119-131`; `middlewares/__init__.py` | Yes (file reads + grep: 0 `update_id`/`allowed_updates`) | Yes — medium (idempotency anchor, no migration required — see Warning #4) |
...
4. **EXT-005 schema prerequisite (no migration for chosen approach):** The **chosen** idempotency approach — `UpdateIdDedupMiddleware` persisting `update_id` in the Django-cache/Redis backing store (not on the `Ad` model) — requires **no** schema change and **no migration** (verified by validator: `update_id` lives in the cache backing store, not the `Ad` schema). The "schema column on `Ad`" prerequisite language above applies **only** to the **rejected** `get_or_create`-keyed-on-`update_id` alternative, which would require a unique constraint / dedup table migration + a `create_draft_ad` signature change and only guards DRAFT creation — not translation/photo side-effects. The middleware approach guards all side-effects centrally with zero schema impact. **No migration prerequisite.**
...
1. **EXT-001 (hard gating prerequisite, MEDIUM):** Before the httpx migration stage, narrow the `except` at `translation.py:170` to explicitly include the `deep_translator.exceptions` family: `(TimeoutError, RequestException, deep_translator.exceptions.TooManyRequests, deep_translator.exceptions.RequestError, deep_translator.exceptions.TranslationNotFound)`. **This is a hard prerequisite —** a bare `(TimeoutError, RequestException)` narrowing omits the deep_translator family (which subclass only `Exception`, not `requests.exceptions.RequestException`) and would let Google 429/non-200/no-element propagate into `translate_all_languages`' `asyncio.gather` (`ad_create.py:1270-1274`), crashing ad creation instead of falling back to the original text. The narrowing **must** ship with the family list; the httpx migration must not land until the family list is in place. Retry `TimeoutError`, `RequestException`, `TooManyRequests` (429), and `RequestError` (non-200); fall back only on `TranslationNotFound` (no-element → same input returns same response, never retry).
...
3. **EXT-004 (low):** Re-add `Strict-Transport-Security` to the `/protected-media/` location (or extract all security headers into an `include`'d snippet replicating the `nginx.conf:66-69` `/static/` pattern) in both nginx configs; add a site-wide `Referrer-Policy: strict-origin-when-cross-origin`.

### Advisory Recommendations

1. **EXT-001:** Add bounded retry with capped exponential backoff at `translate_text`; replace the abandoned-future pattern with a bounded `Semaphore`; **first** narrow the `except` to `(TimeoutError, RequestException, deep_translator.exceptions.TooManyRequests, deep_translator.exceptions.RequestError, deep_translator.exceptions.TranslationNotFound)` (per Warning #1 — do not drop the deep_translator family or ad creation will crash on 429/no-element); split: **retry** `TimeoutError`, `RequestException`, `TooManyRequests` (429), `RequestError` (non-200); **fall back only** on `TranslationNotFound` (no-element → same input returns same response, never retry); migrate to `httpx` with per-request timeout + cancel scope (staged after retry/semaphore). **Prerequisite:** this narrowing (with the family) must ship before the httpx migration.
2. **EXT-002:** Construct **one `Bot` per thread/loop**, reused across the batch and closed once after the batch (NOT a process-global shared `Bot` — `aiohttp.ClientSession` binds to the loop at creation; see Validator report); add `return_exceptions=True` to the `asyncio.gather` at `immediate_alerts.py:195` so one failed `_send` does not abort the whole batch — classify results (retry transient: `TelegramRetryAfter` honouring `retry_after` + 5xx/network; dead-letter permanent: `TelegramForbiddenError`/`TelegramBadRequest`); bound delivery threads globally via a bounded process-level `ThreadPoolExecutor`; add 429 `retry_after` backoff + 5xx/network retry; dead-letter permanently-unreachable chats.
3. **EXT-005:** Add update-level dedup via `UpdateIdDedupMiddleware` (persist `update_id` in the Django-cache/Redis backing store via atomic `cache.add`, fail-open on `ConnectionInterrupted`/`redis.RedisError`, 24 h TTL) registered at `dp.update.middleware()` in `main.py` — **no migration required** for this chosen Approach A (see Warning #4). The `get_or_create`-keyed-on-`update_id` alternative (which DOES require a migration + `create_draft_ad` signature change and only guards DRAFT creation) is rejected; guard translation/photo side-effects via the middleware. (Warning #4)
