...
| Zone | Concern |
|------|---------|
| **Upload Ingestion** | The bot receives photo payloads, first staging them to `staging/<uuid>.jpg`; validates type (magic-bytes)/dimensions/count, then enforces a two-layer size check — (1) pre-download the Telegram `file_size` hint vs `MAX_PHOTO_BYTES` (defense-in-depth), then (2) post-download `validate_photo` (byte-size + 2560px dimension cap + decompression-bomb ceiling); strips EXIF/ICC in `save_photo` (applies `exif_transpose`, drops `exif` + `icc_profile`, re-encodes JPEG with `optimize=True`) before `move_staging_to_permanent` promotes the file; then persists bytes + a photo-collection row tied to the ad. |
| **Storage / Naming** | The media store holds files under a dual key scheme, both validated against `KEY_FORMAT_REGEX` (no embedded ad_id/user/Telegram PII): seeded originals use `seed/<filename>.jpg` (thumbnails `seed/<stem>-small.jpg`); bot uploads are staged as `staging/<uuid>.jpg` (EXIF-stripped), then promoted by `move_staging_to_permanent` to `<uuid>.jpg` (thumbnails `<uuid>-small/medium/large.jpg`). Local store today, contractually swappable to object storage. |
| **Serving / Access Control** | nginx proxies `/media/` to Django (files never served directly by the static layer): the `media_gate` view is the per-request auth gate — it rejects NUL/control chars + path traversal, requires a referencing photo-collection row, and returns 403 for non-staff unless at least one referencing ad is `PUBLISHED` (staff exempt) — then issues `X-Accel-Redirect: /protected-media/{image_key}` to nginx's `internal` `location /protected-media/` (MIME-whitelisted to `image/jpeg`, security headers re-applied); `Cache-Control: no-store` (production) prevents stale serves of deleted photos. (`DEBUG=True` falls back to a `FileResponse`.) |
