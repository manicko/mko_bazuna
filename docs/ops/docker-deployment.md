...
The one-shot services (`migrate`, `load_cities`, `load_catalog`, `create_admin`, `seed`) set `DJANGO_BUILD=1` in their `environment:` blocks, which causes `prod.py` to skip import-time placeholder validation for `BOT_TOKEN` and `GOOGLE_TRANSLATE_API_KEY`. These services receive placeholder/dummy values from `.env.dev` and do not serve web traffic, so the production fail-fast guards are bypassed during bootstrap; the real secret values are enforced at runtime by the long-lived `web` and `bot` services, which never set `DJANGO_BUILD`. See [Deployment Checks](#deployment-checks) for the full validation scope.
