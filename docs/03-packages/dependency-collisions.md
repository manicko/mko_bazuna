...
|------|-------|------------|
| deep-translator Google-scrape fragility | HIGH | Hard timeout ~500ms + circuit breaker (3 failures → 60s cooldown) + LRU cache, governing bot-side ad-creation translation. Mandatory fallback to original text/query on failure. |
