...
| httpx | `>=0.28.0` | (none — HTTP client only) | Stable, officially-supported HTTP client for the Google Cloud Translation API v2 Basic (`apps.core.services.translation`). Socket-level timeout (`httpx.Client(timeout=…)`) truly interrupts hung upstream calls, reclaiming `ThreadPoolExecutor` workers instead of orphaning them. Risk is upstream-API availability/throttling (429/5xx), not client fragility. | `packages-list.md` lines 35, 50, 88, 95; `spec-index.md` line 47 (Bosnian→Russian at ad publication only; search is per-language FTS, no query-time translation) |
