...
| DEP-002 — CI security scanning not enforced | Security job made blocking: pip-audit, Trivy (fs + image), gitleaks now fail the build (`exit-code: 1`, no `continue-on-error`). | `chore(ci): make pip-audit, Trivy, and gitleaks scans blocking and align CI-security tests` (B4) |
