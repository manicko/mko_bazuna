...
- **Login (H):** QR deep-link `login_<token>` (32-char URL-safe token (~192-bit CSPRNG, rate-limited 5-min TTL — exceeds NIST SP 800-63B's 128-bit minimum)), `LoginToken` two-phase atomic claim, `hmac.compare_digest`; consumption via POST body + CSRF (not URL query).
...
| **URL State Preservation** | Hybrid URL model (path-encoded primary context + query-encoded refinements) with `query_replace` template tag for lossless category/city/breadcrumb/pagination navigation; HTMX `hx-push-url` safety nets ensure partial swaps never drop `lang`, `sort`, or filter params; "Entire country" clear preserves lang + non-city filters on all pages; language switching drops `page` only | [`url-state-preservation.md`](url-state-preservation.md), `query_replace` tag, `CityResolutionMiddleware`, `PreferredCityMiddleware`, `LanguagePreMiddleware` |
| **Contact-Us & Bot Username Centralization** | Telegram bot username moved from env var to `SiteConfig` singleton (admin-editable, regex-validated, 1h cached); all deep-link URLs rendered via `telegram_deep_link` template tag with base64 `data-*` JS click-to-reveal + CSS `direction:rtl` display-text obfuscation, no `<noscript>` fallback; dual-layer rate limiting (web per-IP + bot per-TG-user); footer "Contact us" link visible to all visitors with no consent gate; Telegram-bypass "Contact us" button for direct bot openers | [`contact-us.md`](contact-us.md), `SiteConfig.bot_username`, `telegram_deep_link` tag, `rtl_obfuscate` filter, `check_contact_start_rate_limit`, `apps/core/migrations/0003_add_bot_username.py` |
...
| **17** | URL state (lang, city) lost on category navigation; city dropdown not linked to URL state | [Complete](.ai/plans/done/17_url-state-preservation_spec_DONE.md) — Signed-off (PO confirmed Q1=A, Q2=A, Q3=A, Q4=A). See [url-state-preservation.md](url-state-preservation.md) for the consolidated spec. |
| **18** | No contact-us footer link; bot username in cleartext env var (not admin-configurable); no anti-spam protection for Telegram deep-links | [Signed-off](.ai/plans/done/18_contact-us_spec.md) — PO confirmed Q1=A, Q2=A, Q3=A, Q4=A, Q5=A, Q6=A, Q7=A, Q8=C. See [contact-us.md](contact-us.md) for the consolidated spec. |
